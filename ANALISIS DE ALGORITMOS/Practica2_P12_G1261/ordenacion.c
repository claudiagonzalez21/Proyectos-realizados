...
introsort(int *tabla, int ip, int iu, pfunc_pivote pivote, int prof){
aux = (ip+iu)/2;
  if(prof == 0){
      mergesort(tabla, ip, iu, piv);
    }else{
      introsort1(tabla,ip,aux,pivote,prof);
      introsort1(tabla,aux+1,iu,pivote,prof);
      quicksort(tabla, ip, iu,piv);
    }
  return ob;
}
*/
...
int mergesort(int* tabla, int ip, int iu){
  ...
  if (ip==iu){
      ...
      aux = (ip+iu)/2;
      if ((ob1 += mergesort(tabla, ip, aux)) == ERR) return ERR;
      if ((ob2 += mergesort(tabla, aux+1, iu)) == ERR) return ERR;
      return   ob1 + ob2 + merge(tabla,ip,iu,aux);
    }
...
int merge(int* tabla, int ip, int iu, int imedio){
 ...
 tamanio= iu+1;
  ...
  while (i<=imedio && j<= iu){
    if(tabla[i]<tabla[j]){ /*SI CAMBIAS EL < ORDENA DE MAYOR A MENOR*/
      taux[k]=tabla[i];
      i++;
...
int partir(int* tabla, int ip, int iu,int *pos){
  ...
  for(i=ip+1; i<=iu; i++){
    ob++;
    if (tabla[i]>tabla[m]){/*SI CAMBIAS EL < ORDENA DE MAYOR A MENOR*/
      (*pos)++;
      aux=tabla[i];
      tabla[i]=tabla[*pos];
      tabla[*pos]=aux;
